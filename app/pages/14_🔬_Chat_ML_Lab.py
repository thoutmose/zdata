...
if "chatml_loaded" not in st.session_state:
    st.session_state["chatml_loaded"] = False

st.info(t("chatml.load_explanation"))
if st.button(t("chatml.load_button"), key="chatml_run_all", type="primary"):
    st.session_state["chatml_loaded"] = True

if st.session_state["chatml_loaded"]:
    ...
    streamers_k = st.slider(
        t("chatml.streamers_n_clusters"),
        min_value=2,
        max_value=8,
        value=5,
        key="chatml_streamers_k",
    )
    ...
    if st.button(t("chatml.classify_button"), key="chatml_run_classify"):
        with st.spinner(t("chatml.classify_spinner")):
            ...
            if combined.is_empty():
                ...
                lexicon_col = classified["lexicon_hostile"]
                ml_col = classified["ml_toxicity_label"] == "toxic"
                matches = lexicon_col == ml_col
    ...
    if date_range is None:
        ...
        if forecast_span.total_seconds() <= 0:
            ...
            if len(forecast_features) < 10:
                ...
                forecast_result = fit_donation_forecast(
                    forecast_features, forecast_cols, "amount_eur"
                )
